 ...
 * @return {String}
 */
const absolutePathDist = (file, buildDir) => {
	let distPath = path.relative(path.resolve(__dirname, '..'), file);
	...
	baseDirs.some(dir => {
		const matches = distPath.indexOf(dir) === 0;

		if (matches) {
			distPath = path.relative(dir, distPath);
		}
 ...
 * @type  {String}
 */
const copyAsset = (asset, dir, destBase) => {
	const src = asset.absolutePath;
	const dist = absolutePathDist(src, destBase);
		...
		require('postcss-url')([
			{
				filter: /\.(woff2?)$/,
				url: (asset, dir) =>
					copyAsset(asset, dir, utils.buildFontsPath())
			},

			{
				filter: /\.(jpe?g|png|gif|svg)$/,
				url: (asset, dir) =>
					copyAsset(asset, dir, utils.buildImagesPath())
			}
		])
	);
